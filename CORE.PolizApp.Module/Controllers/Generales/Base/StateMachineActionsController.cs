...
namespace DevExpress.ExpressApp.StateMachine
{
  public class StateMachineActionsController : StateMachineControllerBase< ObjectView >
  {
    ...
    private readonly Dictionary< object, List< SimpleAction > > panelActions = new Dictionary< object, List< SimpleAction > >( );
    private readonly SingleChoiceAction stateMachineAction;
    private IList< ITransition > originalTransitionsList;

    public StateMachineActionsController( )
    {
      stateMachineAction = new SingleChoiceAction( this, "CustomChangeStateAction", PredefinedCategory.Edit );
      stateMachineAction.ImageName = "Action_StateMachine";
      stateMachineAction.PaintStyle = ActionItemPaintStyle.Image;
      stateMachineAction.Caption = "Change State";
      stateMachineAction.ToolTip = "Change state of the current object";
      stateMachineAction.ItemType = SingleChoiceActionItemType.ItemIsOperation;
      stateMachineAction.SelectionDependencyType = SelectionDependencyType.RequireSingleObject;
      stateMachineAction.Execute += stateMachineActivity_Execute;
    }

    public SingleChoiceAction ChangeStateAction
    {
      get { return stateMachineAction; }
    }
    ...
    private void IntializeActionsPanel( IStateMachine stateMachine, ChoiceActionItemCollection transitionItems )
    {
      ...
      {
        SimpleAction transitionAction = CreateSimpleTransitionAction( stateMachine, transitionItem );
