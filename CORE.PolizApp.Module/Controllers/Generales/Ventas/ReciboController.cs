...
namespace CORE.Modulos.Controllers.Ventas
{
  public class ReciboController : PagoController
  {
    ...
    protected override void GenerarPagoAnticipo( Dictionary< int, decimal > saldos )
    {
      ...
      var ultimoNumero = session.Evaluate< Comprobante >( CriteriaOperator.Parse( "MAX(Numero)" ), CriteriaOperator.Parse( "Destinatario = ? AND Tipo = ? AND Sector = 1", destinatario, anticipoComprobanteTipo ) );
      ...
      comprobanteItem.PrecioUnitario = valorTotalLocalAnticipo * -1; //el calculo de saldos fue aplicaciones - valores. Por lo tanto el excedente de valores está negativo, así que invertir signo
